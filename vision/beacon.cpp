...
int main(int argc, char **argv)
  {
  const string filename1("./beaconON.jpg");
  const string filename2("./beaconOFF.jpg");
...
while(true){
  ...
  imgON = imread(filename1, CV_LOAD_IMAGE_COLOR);
  imgOFF = imread(filename2, CV_LOAD_IMAGE_COLOR);

//cap>>img;
cout<<imgON.rows<<" "<<imgOFF.cols<<endl;
  if(img.empty()){
