...
void tennisCallback( const std_msgs::Float64MultiArray& angle)
{
  int dir;

  double ang = angle.data[0];
  double dist = angle.data[1];
  ...
  if(ang > -5 && ang < 5 && dist < 10){
  robot.movexinches(24, 24);
  }
}
...
ros::Subscriber<std_msgs::Float64MultiArray> sub("tennisball_angle",&tennisCallback); 
